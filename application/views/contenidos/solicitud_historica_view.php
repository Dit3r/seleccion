	...
	<div class="row">
		...
		<div class="col-md-4" style="margin-top:10px; padding-left:8px;">		
			<span class="btn btn-default btnEnlace"><i class="glyphicon glyphicon-download-alt"></i>
			    <a href="<?php echo site_url(); ?>traslado/genLstFinCsv">Descargar excel</a>
			</span>
		</div>			
...
foreach ($filas as $item): 
		...
		$strQueryTerminado = "select pkg_seleccion.genera_string_detalle(".$item->WF_KEY.") as STRING_ETAPAS from dual";
...
/*							indica quien es el actual responsable
							...
							if (isset($_SESSION["rut"])) {
								if ($item->RESPONSABLE == "888") {
									...
									if ($strQueryValJef[0]->VALIDA_JEFATURA == "S") {
										$cantidadVerdes++;
						?>
									<a style="color:#4CAE4C" title="<?php echo $descEstLarga[$contador][0]->EST_DESC_LARGA; $contador++; ?>" class="hastip verde" href="<?php echo base_url()?>etapa/historico/<?php echo $item->WF_KEY; ?>/<?php echo $item->ETAPA_ACTUAL; ?>/<?php echo $item->CORRELATIVO; ?>/<?php echo $item->ID_FLUJO; ?>" original-title="Hello World">
						...
						?>
									<a style="color:#E65D11" title="<?php echo $descEstLarga[$contador][0]->EST_DESC_LARGA; $contador++; ?>" class="hastip rojo" href="<?php echo base_url()?>etapa/historico/<?php echo $item->WF_KEY; ?>/<?php echo $item->ETAPA_ACTUAL; ?>/<?php echo $item->CORRELATIVO; ?>/<?php echo $item->ID_FLUJO; ?>" original-title="Hello World">
										<span class="glyphicon glyphicon-info-sign"></span>
										<?php echo $item->ESTADO; ?>
									</a>						
						<?php
